# LumioGameEngine V3 (v1.4) 架构与开发规范
...
> **v1.3 → v1.4 变更**：契约归一波次（ADR-037/038/039）。公共原语下沉 `common.schema.json`（恢复记录链、PackageIdentity 五元组、SignatureBlock、SessionReleaseTriple、TrustDomain、CompressionCodec、StateTransitionEvent）；缺陷修复冻结（ChunkId 键 `c:x:y:z` 全仓统一、Snapshot checksum 收紧为 SHA-256、Replication integrity 按算法分支约束、Gate 词汇对齐 ID Registry、abortReason 交集冻结）；新增 `state-machine-descriptor` P0 契约并把 12 个架构状态机登记为可生成描述符；ADR-023 Artifact 类别新增 `ContractRuntime`（纯 Rust crate + 纯 C# assembly 支持库，零 Native 依赖）。Voxel 族 ADR-024/025/035/036 随本基线正式落定归属。v1.3 正文保留为历史基线。
> **v1.2 → v1.3 变更**：闭合 Runtime 审查 Wave 1/2（RT-AR-001/002/004/007/008/009/010/011/012/013/014/019）。CommandBuffer 与参与者标记改为枚举、Tick 冻结 Fail-stop 与 13 相契约矩阵、Replication Envelope 与 typed body 分离并补齐 MessageType、Entity `namespace` 必填、Processor 改为 `mayEmitStructuralCommands` 且允许自重叠、GAS 通用状态机冻结、TxnJournal/CommandLog/WAL 恢复记录独立于 LoggingEvent、Config typed columns 动态校验、Hot Reload 双 Scope 原子激活、FailureBundle 允许 pre-snapshot 条件必填。v1.2 正文保留为历史基线。
> **v1.1 → v1.2 变更**：补齐客户端权威更新单一 Runtime 事务（ADR-021）、生成的 Active 消息 Protocol/Permission 门字段集（ADR-022）、生成契约 Artifact 由本仓工具链发布且零依赖于 Client/Game 实现（ADR-023）。明确 V1 不提供 Session Resume Token、Active Session 内禁止跨 Release 替换 Gameplay Scope。
...
V1 冻结 **Fail-stop**（ADR-027）：Processor 可以原地写已有 Component 字段。任一提交前故障（Processor 异常、取消或超预算中止）使当前 World 不可继续使用，`SimulationSession` 进入 `Faulted`，从 Tick 前 Snapshot 加 Journal 重建。不提供字段级撤销。唯一权威 Tick Commit Point 是 `GasAndEventFinalize`。该点之前的写入对 Replication/Snapshot/Egress 不可见；该点之后的重复 Tick 必须幂等得到同一 Tick Result。
...
mayEmitStructuralCommands, Before/After Dependencies,
DeterminismClass, Budget, DiagnosticName
```

业务 Phase（`ApplyInputs`、`ProcessorPlan`、`CrossWorldPrepare`、`CommitDecision`、`GasAndEventFinalize`）可以声明并发出结构命令；只有 Runtime Commit Executor 在 `EcsCommandBufferCommit` 实际应用。同一 Processor 的 ReadSet/WriteSet 自重叠合法。Scheduler 只校验 Processor **之间** 的冲突和依赖环，不拒绝自读自写。Archetype/Storage 布局不属于公共契约。
...
`tick-phase-contract.schema.json` 冻结 13 相的输入、可写状态域、失败分类、取消点、超预算动作、对后续相可见性、重复 Tick 幂等结果和唯一 Commit Point 标志。权威摘要：

| Phase | 可写域 | 失败分类 | 取消点 | 超预算 | 后续可见性 | Commit Point |
| --- | --- | --- | --- | --- | --- | --- |
| IngressCapture | IngressQueue | ProcessFault | BeforeCommit | FailStop | WithinTickPrivate | 否 |
| DecodeAndCanonicalize | CanonicalCommandSet | BusinessReject | BeforeCommit | FailStop | WithinTickPrivate | 否 |
| ApplyInputs | InputApplySet | BusinessReject | BeforeCommit | FailStop | WithinTickPrivate | 否 |
| ProcessorPlan | ProcessorPlan | SessionFault | BeforeCommit | FailStop | WithinTickPrivate | 否 |
| CrossWorldPrepare | PreparedGameDelta | BusinessReject | BeforeCommit | FailStop | WithinTickPrivate | 否 |
| NativeJobBarrier | NativeCompletions | ProcessFault | BeforeCommit | FailStop | WithinTickPrivate | 否 |
| CommitDecision | CommitIntent | SessionFault | BeforeCommit | FailStop | WithinTickPrivate | 否 |
| VoxelCommit | VoxelWorld | ProcessFault | NotCancellable | FailStop | WithinTickPrivate | 否 |
| EcsCommandBufferCommit | GameWorld | ProcessFault | NotCancellable | FailStop | WithinTickPrivate | 否 |
| GasAndEventFinalize | GasEvents | ProcessFault | NotCancellable | FailStop | AfterCommit | **是** |
| ReplicationProjection | ReplicationView | ProcessFault | NotCancellable | FailStop | AfterCommit | 否 |
| SnapshotHashMetrics | SnapshotHash | ProcessFault | NotCancellable | FailStop | AfterCommit | 否 |
| EgressPublish | EgressQueue | ProcessFault | NotCancellable | FailStop | AfterCommit | 否 |

重复同一 Tick 的幂等结果一律为 `IdempotentSame`。矩阵不得把 Commit Point 标在其他相，也不得让 Commit Point 之前的相声明 `AfterCommit` 可见性。
...
- `namespace` 必填：`Provisional` 必须使用 `client-` authority domain；`Authoritative` 禁止 provisional domain；`Replay` 保留原 ID 时必须携带 `sourceRevision` 与 `sourceReleaseId`。
...
Processor 只能写自己的 CommandBuffer。CommandBuffer 状态机为 `Open -> Sealed -> Merged -> Prepared -> Applied`。ECS 参与者在 `CrossWorldPrepare` 完成 Generation、目标存在性、组件容量、命令冲突、权限和预算校验，产出不可变 `PreparedGameDelta`。结构变化只由 Runtime Commit Executor 在 `EcsCommandBufferCommit` 应用。必须定义 Deferred Entity Token、同 Tick Create/Write/Destroy、无效目标、重复命令和稳定合并规则。
...
Prepared -> Aborted
Prepared -> Expired
CommitIntent / Apply -> Indeterminate
```

CommandBuffer：`Open -> Sealed -> Merged -> Prepared -> Applied`。

参与者标记：`NotStarted | Unknown | Applied | Failed`（不是 Boolean）。

规则：

1. Prepare 只做验证和有租约的 Reservation，不产生可见业务副作用。ECS 在 `CrossWorldPrepare` 完成全部业务校验并产出不可变 `PreparedGameDelta`。
2. 所有可能失败的检查、容量检查和 Chunk 可用性检查都在 Prepare 完成。
3. Coordinator 在固定 Barrier 决定 Commit；`CommitIntent` 持久化之后，ECS Apply 只能返回 `Applied / AlreadyApplied / Indeterminate / Faulted`，不得业务拒绝。
...
6. `TxnJournal` 是独立恢复记录（ADR-032），不是 `LoggingEvent`。结果丢失时用状态查询解决 `Indeterminate`。
...
9. `Indeterminate` 只能从已持久化 `CommitIntent` 的 Apply 阶段进入。pre-intent 失败走 `Prepared -> Aborted/Expired`。
10. Apply 成功但 marker 未落盘：查询参与者收据/状态。`Applied`/`AlreadyApplied` 写回 `Applied` 且不得再 Apply；`Unknown` 保持 `Indeterminate` 直到后续查询或 journal 重放；`Failed`/`Faulted` 保持 `Indeterminate` 并从 Snapshot + journal 恢复。
...
Envelope 与 typed `body` 分离。Envelope 必须包含版本、长度、序号、Session/Release、消息类型、可靠性、完整性校验、TraceId 和 `transportPolicy`（`maxMessageBytes`、`maxFragmentBytes`、`antiReplayWindow`、`authBinding`、三类错误 `Retryable | Rejectable | Fatal`）。`MessageType` 为 `Handshake`、`FullSnapshot`、`BaselineAck`、`Delta`、`DeltaAck`、`ResyncRequest`、`MaintenanceKick`、`Error`，且必须与 ID Registry 及 Fixture 实际使用集合一致。

FullSnapshot body 必填 `TickId`、完整 `SessionRevisionVector`、`SchemaEpoch`、`MappingSetHash`。Delta body 必填 `BaseSnapshotId`、`From/To Revision`、`MappingSetHash`、确认序列和 Tombstone 信息。
...
Ability 实例状态名冻结为：`Requested -> Activated -> Executing -> Completed`；`Requested/Activated -> Rejected`；任一非终态 -> `Cancelled`；`Executing -> Expired`；预测实例被权威拒绝 -> `RolledBack`。终态使 Handle 失效。

Effect 实例状态名冻结为：`Pending -> Active -> Expired | Removed`；`Pending -> Rejected`；预测回滚 -> `RolledBack`。Stack/Duration/Refresh 是 `Active` 内事件，不是独立状态。

Game 内容只能在 Ability `Executing` 或 Effect `Active` 内定义业务子状态，不得增删通用状态名，也不得改变通用转移、终止语义、回滚窗口与 Handle 失效规则（ADR-031）。
...
- 恢复输入是独立的 `TxnJournalRecord`、`CommandLogRecord` 和可选 `WalRecordEnvelope`（ADR-032）：每条记录携带 RecordVersion、RecordSeq、Session/Release/Tick/Txn 或 Command 关联、RecordKind、IdempotencyKey、PreviousHash、PayloadHash、Length、Commit/Durability 状态和 Checksum。`LoggingEvent` 的 `TxnJournal`/`CommandLog` 类别只是诊断镜像，不得充当恢复记录。
...
人类可读源文件经 Schema 校验、默认值合并和编译，运行时通过 GameRuntime **Config Port** 完成 typed materialization：版本化 Table Reader 读取 typed binary table，行/键 ID、类型、范围和引用在激活前校验。`columns` 与 `activation` 必填；Architecture Gate 按列描述动态校验类型、范围、枚举、引用目标、未知列和默认值（默认值不能补造缺失的必填单元格）。层级顺序固定为 Engine → Platform → Server → Product → Environment → User/Session。Port 持有 Staged 与 Active 两份不可变 `ConfigSnapshot`，以 `ConfigRevision` 标识；生产切换只在 Tick Barrier 原子激活（`activation` 为 `ProductionSignedSwitch` 时必须带签名）。开发可热载，生产只能通过带 Hash/签名的版本切换生效；Secret 与普通配表分离。谁在何时请求 staging 属于宿主编排，不进入本 Port 契约。
...
每个事件声明 `correlation.scope`（`Process`/`Release`/`Session`/`World`/`Txn`），表示事件合法持有的最深身份层级。基础字段 `ProductId、GameReleaseId、TraceId、ProducerId、EventSeq` 恒必填；`SessionId`、`WorldId/TickId`、`TxnId` 只在对应作用域必填，进程启动、Manifest 校验、认证拒绝等早期事件不得伪造尚不存在的 ID。Failure Bundle 声明 `incidentKind`（`Simulation`/`CoreEngineLoad`/`SupplyChain`/`BuildValidation`）：`Simulation` 事件必须引用 `snapshotId`，或携带 `noSnapshotReason + bootstrapPhase + lastKnownRevision/manifest` 以表示首次有效 Snapshot 之前的故障；`CoreEngineLoad`/`SupplyChain` 事件必须携带 `coreEngine` 块（PackageIdentity、Loader 状态、ErrorCode、Trust 决策）且作用域为 `Process`——加载期失败发生在任何 Session/World 存在之前，SnapshotId 对其改为不适用而非必填。`ReleasePoolId、MaintenanceId、WorldSlotId、NetEntityId、PredictionKey、SnapshotId` 等在适用时附加。异步 Sink 不承诺跨线程的实时全局顺序，但必须保留每个 Producer 的 `EventSeq` 和 Tick 关联以便重建；日志不能替代 Txn Journal 或 Command Log。网络时间戳和队列状态进入 Diagnostic Hash，不进入权威 Simulation Hash。
...
同 Release 模块热更使用双 Scope 状态机（ADR-034）：

```text
OldActive + NewStaging -> NewValidated -> BarrierSwitch -> OldQuiescing -> OldUnloaded
```

BarrierSwitch 前失败：丢弃 `NewStaging`，`OldActive` 不变。切换后失败：不得重新激活已排空或 Dispose 的旧 Scope，Session 进入 `Faulted` 并从有效 Snapshot/Release 恢复。Migration 只读不可变 Snapshot，在 Staging 生成结果。入口、订阅、Timer、Task 和 Native Lease 按 Scope Generation 在切换点线性化。Migration Manifest 必须记录同一套 `scopeActivation`。
