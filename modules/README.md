...
> **分层命名**：16×16×16 = 4096 格的数据单元叫 **Section**（`SectionId`、`SectionRevision`），是最小同步单位、驻留单位和版本锚点；**Chunk** 是 16 个 Section 竖着摞成的列容器（16×256×16），只作存档打包、按列计算和流式批量请求的单位，不携带数据、不持有独立 revision。唯一真值是活契约 `lumio.voxel-world.v1`（[0013](../.spec/decisions/0013-voxel-world-contract-and-section-rename.md)、[`features/voxel-section-chunk.md`](../.spec/knowledge/features/voxel-section-chunk.md)）；死基线产物 id `voxel-chunk-page`、`VoxelChunkResidency` 是冻结字符串，不随之改名。
...
1. **总体方向可进入模块化设计阶段**：World/Section/Revision 的所有权、Server/Client 双实例隔离、Runtime Port 边界和 NativeCore 依赖方向一致。
2. **根 README 与架构源的粒度对齐**：架构源 §16 首批清单现为 `world/chunk/revision/query/mutation/snapshot/streaming`（其中的 `chunk` 即本仓的 `section` 模块——16³ 数据单元按活契约叫 Section，死基线的清单不会再重发）；`spatial`、`migration` 和 `mesh-collision` 仍是本仓实现粒度细化，不改变仓库所有权。
3. **`query` 必须独立**：只读批量查询拥有独立的预算、取消、缺 Section 结果和读取 Revision 语义，不能隐藏在 `section` 或 `world` 内部。
4. **`spatial` 与 `mesh-collision` 不等于 NativeCore 的通用 Kernel**：本仓只负责把 Section/Block/遮挡数据投影为 Voxel 领域结果；通用空间、碰撞和压缩算法仍归 NativeCore。投影只经 ReadView，不直连 Section Storage。
5. **`migration` 不在 Tick 热路径**：它提供 Voxel 节点语义；完整 DAG、Staging 目录、Checkpoint 索引和原子激活归 Host/Server。失败不得改写旧数据。
6. **公共 Voxel 契约已在 `LGE-V1.4-2026-08-27` 发布**：World/Port、Section 页、Revision Stamp、Query、Mutation receipt 以及 Snapshot payload（ADR-035）与 Streaming DurabilityAck（ADR-036）均以架构源为准；模块 README 不再复制字段布局。Spatial/Mesh 仍无跨仓 Schema。

## 2. 系统上下文与仓库边界

LumioVoxelEngine 是七仓库体系中的 VoxelWorld 领域实现（架构源 §2.1）：

- **本仓拥有**：VoxelWorld、Chunk / Section / Block 分层与坐标、World/Section Revision、Query、Mutation、Voxel Snapshot/Diff payload、Streaming、Voxel Migration 节点，以及 Voxel Spatial/Collision Source。
...
4. Runtime/Server/Client 只能消费版本化 `IVoxelWorldPort` 和生成契约，不能读取内部 Section Storage。
...
| [section](section/README.md) | Section 坐标、Block 存储、页布局、页编码和驻留状态 | L1 基础数据 | `lumio-voxel-domain` | P0 |
| [revision](revision/README.md) | World/Section Revision、比较、读取令牌和 Snapshot Pin/COW 记录 | L1 基础数据 | `lumio-voxel-domain` | P0 |
| [query](query/README.md) | 有界只读批量查询、缺 Section 结果、读取 Revision 和取消 | L3 领域 API | `lumio-voxel-ops` | P0 |
...
| [streaming](streaming/README.md) | Section Load/Unload、优先级、预算、取消、背压和可用性 | L3 生命周期服务 | `lumio-voxel-ops` | P2 |
...
`A --> B` 表示 A 编译/API 依赖 B。`section` 与 `revision` 是 sibling，不互调。`world` 是组合根；L0–L4 与 Tool 不得依赖 `world`。不存在指向 `LumioCoreEngine` 的编译边。
...
- `streaming` 负责请求调度和 Section 可用性任务，不拥有 World 生命周期；`world` 决定何时启动、排空和关闭它。
...
IVoxelWorldPort.prepare / commit / abort
  -> world
  -> mutation
  -> CommitBatch publish { section pages, Dirty, SectionRevisionSet, WorldRevision }
...
Restore
  ...
  -> snapshot.decode
  -> world restore entry
  -> section.materialize_pages + revision.restore_stamps
  (not streaming Load)
...
1. **创建**：Host 分配句柄和 Capability → `world` 建立实例上下文 → 初始化 `revision/section` → 注册 `query/mutation/snapshot` → 按能力挂接 P2 `streaming/spatial/mesh-collision` → 返回 `VoxelWorldHandle`。
...
3. **单域修改**：Runtime Barrier → `world` 转交 `mutation.prepare` → 校验 Section/Cell/Expected Revision 并创建不可见 Reservation → Coordinator 或单域调用方决定 Commit → `mutation.commit` 经 CommitBatch 原子发布页与 Revision → 返回新 World/Section Revision。
...
6. **Streaming**：上层提交带优先级和截止条件的 Load/Unload → `streaming` 放入有界队列 → IO/解压完成后在 Barrier 发布 Section 可用性 → `query` 明确返回契约四态 `Ready/Unchanged/Pending/Unavailable`。
...
9. **恢复**：Host 提供不可变字节 → `snapshot.decode` → `world` restore 入口 → `section` 物化页、`revision` 恢复 Stamp。
10. **销毁**：先关闭 Ingress 并停止新写入 → 完成/中止 Reservation → 导出诊断与 Snapshot 元数据（向 Host，不在 world 内缓存 Cut）→ 停止 Streaming/构建任务 → 释放 Section/Revision → 使所有旧 Handle 失效。
...
| Section/Block/坐标/页布局/加载态/Dirty | `section` | Storage 私有；Dirty 仅在 Host DurabilityAck 后由 Barrier 清除 |
| WorldRevision、SectionRevision、读取令牌、Pin/COW 记录 | `revision` | 不允许无语义的统一整数替代域 Revision；不拥有 Block 页 |
...
故障按最小影响范围处理：单次 Query/Mutation → Section/Streaming → World 实例 → 进程。连接、Session、Release Pool 和进程级重启不在本仓裁决；本仓必须提供稳定错误、Revision、Failure Bundle 片段和可重放输入。
...
- `streaming` 的 IO/解压线程只生产完成事件；Section 状态转换在 Barrier 发布，避免后台线程直接改权威状态。
...
| `NativeManagedAbiV1` | `schemas/native-managed-abi.schema.json` | world、section、snapshot、streaming |
...
| ID Registry | `ids/index.json` | world、section、revision、mutation |
| Voxel World/Port | `schemas/voxel-world-port.schema.json` | world |
| Voxel Section 页 | `schemas/voxel-chunk-page.schema.json` | section、query、streaming |
...
| Voxel DurabilityAck | `schemas/voxel-durability-ack.schema.json`（ADR-036，随 `LGE-V1.4-2026-08-27` 冻结） | streaming、section |

模块 README 不得复制上述 Schema 的字段布局；只引用 `$id` 与本仓方法名。Spatial/Mesh 仍无跨仓 Schema。`voxel-chunk-page` 是死基线冻结的 schema id，不随 16³ 数据单元改名 Section 而改变（见 `lumio_voxel_contracts::legacy_baseline`）。
...
| --- | --- | --- | --- | --- |
| VOX-D-001 | Section 数值 profile（维度、世界边界、页大小） | 线格式已由架构源 ADR-024 冻结；消费者仍不得依赖具体尺寸 | section、query | Bench 卡确认数值，写入配置快照 |
| VOX-D-002 | Block 存储与压缩后端 | 通过 Adapter/页接口隔离，优先 OSS，保持确定性 | section | 密度、CPU、内存和许可证 Benchmark |
| VOX-D-003 | Query 批次、预算默认值 | 一致性与缺 Section 四态已由架构源 ADR-024 / `voxel-query` 冻结；容量仍开放 | query、streaming | Bench 后写入配置快照 |
| VOX-D-004 | Reservation 租约与 receipt 表容量 | 崩溃恢复与 pruning 已由架构源 ADR-025 冻结；租约/表上限仍开放 | mutation | NativeHeadless 容量与活性测试 |
| VOX-D-005 | Snapshot Pin/COW 与子 Section Diff 粒度 | 载荷线格式已由架构源 ADR-035 交付；物化策略仍开放（D-014） | snapshot、revision | 内存放大与 Diff 体积基准 |
...
1. **Architecture Gate**：消费 `LGE-V1.4-2026-08-27` 已冻结的 World/Section/Revision/Mutation/Snapshot 公共语义、Voxel 专属 Schema、正向/失败 Fixture 和 Port 错误表。
2. **Foundation**：实现 `revision/section/world/query/mutation/snapshot` 的单域闭环，Reference Port 和 NativeHeadless 语义测试可运行。P0 Profile 可全驻留并禁用 Dirty Unload。
...
- LocalEmbedded 双实例、Revision 一致性、缺 Section、Prepare/Commit 顺序和 Snapshot Cut 的边界没有互相矛盾。
