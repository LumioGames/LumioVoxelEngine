...
`query` 是所有上层读取 Voxel 状态的唯一领域 API。它把坐标范围、Chunk 可用性、读取一致性、批次上限和取消语义组合成可审计的结果，供 Runtime、Spatial、Mesh/Collision 和 Headless 测试使用。它永远不产生可见写入，也不把内部 Storage 复制成 C# 第二真相。
...
- 不加载或驱逐 Chunk（归 [streaming](../streaming/README.md)），不修改 Block（归 [mutation](../mutation/README.md)）。
- 不拥有 World/Chunk 生命周期或 Revision 递增（归 [world](../world/README.md)、[revision](../revision/README.md)）。
...
- Query 诊断计数、缺 Chunk 分类和读取 Revision 记录。
...
- Diagnostic：请求拒绝、缺 Chunk 分类、超时/取消、QueueFull、读取 Revision。
...
- **测试面**：点/范围/批量查询、边界坐标、缺 Chunk 三态、Revision 传播、批次上限、取消/超时幂等、结果稳定排序。
- **故障矩阵**：Chunk 在读取中卸载、Generation 复用、QueueFull、解压失败、OOM、旧 Revision、Local 双实例串读。
